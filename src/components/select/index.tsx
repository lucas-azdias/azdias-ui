...
import type {
    ...
    SelectTriggerProps,
    SelectValueProps,
} from "@azdias/ui/components/select/types";

export function Select<Value, Multiple extends boolean | undefined = undefined>(
...
export function SelectTrigger({ className, size = "default", children, ...props }: SelectTriggerProps) {
    return (
        <SelectPrimitive.Trigger
            ...
            data-size={size}
            className={cn(
                "flex w-fit items-center justify-between gap-1.5 rounded-lg border border-input bg-transparent py-2 "
                + "pr-2 pl-2.5 text-sm whitespace-nowrap transition-colors outline-none select-none "
                ...
                + "[&_svg:not([class*='size-'])]:size-4 focus-visible:outline-solid",
                className
            )}
            {...props}
        >
    ...
    return (
        <SelectPrimitive.Item
            ...
            className={cn(
                ...
                + "not-data-[variant=destructive]:focus:**:text-accent-foreground data-disabled:pointer-events-none "
